...
echo "Provisioning emulator for aosp_backup..."
...
if [ ! -f backup.tar.gz ]; then
  ...
  wget --quiet https://github.com/aosp_backup-app/aosp_backup-test-data/releases/download/3/backup.tar.gz
fi
...
$ADB wait-for-device
$ADB shell mkdir -p /sdcard/aosp_backup_baseline
$ADB shell tar xzf /sdcard/backup.tar.gz --directory=/sdcard/aosp_backup_baseline
$ADB shell rm /sdcard/backup.tar.gz

echo "Emulator '$EMULATOR_NAME' has been provisioned with aosp_backup!"
